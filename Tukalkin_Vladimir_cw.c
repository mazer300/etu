		...
		case 180:
			...
			for (int i = 0; i < width; i++) {
				for (int j = 0; j < height; j++) {
					...
					// Rotate the pixel by the given angle.
					switch ((int)angle) {
						case 90:
							x = j;
							y = width - 1 - i;
					...
					rotated_image[(j * width + i) * 4] = pixel[0];
					rotated_image[(j * width + i) * 4 + 1] = pixel[1];
					rotated_image[(j * width + i) * 4 + 2] = pixel[2];
				}
			}

			// Free the memory used by the original image.
			free(image);

			// Update the image with the rotated image.
			image->row_pointers = rotated_image;
			...
			int flag=0;
			if((right_down[0]-left_up[0])%2==0) flag=1;
			
			for(int y=left_up[1];y<right_down[1];y++){
				for(int x=left_up[0];x<right_down[0]/2+1-flag;x++){
					int x3=width-x-flag;
					int y3=height-y-1;
					
					int black[3]={0,0,0};
					
					int color1[3]={image->row_pointers[y][x*3],image->row_pointers[y][x*3+1],image->row_pointers[y][x*3+2]};
					int color2[3]={image->row_pointers[y3][x3*3],image->row_pointers[y3][x3*3+1],image->row_pointers[y3][x3*3+2]};
					
					set_pixel(image,x,y,color2);
					set_pixel(image,x3,y3,color1);
			...
			for(int y=0;y<image->height;y++){
				for(int x=0;x<image->width/2+1-flag;x++){
					int x3=image->width-x-flag;
					int y3=image->height-y-1;
