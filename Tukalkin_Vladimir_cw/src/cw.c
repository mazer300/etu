...
void reference();
void function_number_9(char* text[], int count_sentences, int number);
...
int main(){
	...
	int number_9;
	if(command_number=='9') scanf("%d",&number_9);
		...
		if(flag_mark==0){
			switch (command_number){
				...
				case '4':
					...
					break;
				case '9':
					function_number_9(text, count_sentences, number_9);
					break;
			}
		}
...
void function_number_9(char* text[], int count_sentences, int num){
	for(int i=0;i<count_sentences;i++){
		int count=0;
		while(count<num-1){
			printf("%s ",text[i]);
