...
class AhoCorasicAlgorithm:
    ...
    def search(self, text):
        ...
        print("\nНачинаем поиск")
        results = []  # массив с результатом поиска
        currentNode = self.root  # начинаем с корня
        ...
        for position in range(len(text)):
            ...
            while temp:
                ...
                for patternIndex in temp.patternIndices:  # перебираем индексы шаблонов в узле
    ...
    def __printAutomat(self):
        ...
        while queue:
            ...
            childrens = ", ".join(
                [f"'{char}':{node.id}" for char, node in node.childrens.items()])  # информация об узлах_детях
...
def findSubPatterns(pattern: str, jokerSymbol: str):
    ...
    subPatterns = []  # список для хранения найденных подшаблонов
    positions = []  # список для хранения стартовых позиций подшаблонов
    current = ""  # текущий формируемый подшаблон
    startPosition = 0  # начальная позиция текущего подшаблона

    print(f"\nНачинаем разбиение шаблона '{pattern}' с джокером '{jokerSymbol}':")

    for i, char in enumerate(pattern):
        print(f"\nОбрабатываем символ {i + 1} ('{char}'):")
        # если символ не джокер - добавляем к текущему подшаблону
        if char != jokerSymbol:
            current += char
            print(f"    Символ не джокер, добавляем к текущему подшаблону: '{current}'")
        # если встретили джокер - сохраняем текущий подшаблон (если он не пустой)
        else:
            if current:
                print(f"    Встретили джокер, сохраняем подшаблон '{current}' (позиция {startPosition})")
                subPatterns.append(current)  # добавляем подшаблон
                positions.append(startPosition)  # добавляем позицию подшаблона
                current = ""
            else:
                print(f"    Встретили джокер, но текущий подшаблон пуст - пропускаем")
            # следующий подшаблон начнется после этого джокера
            startPosition = i + 1
            print(f"    Новая стартовая позиция для следующего подшаблона: {startPosition}")

    # после цикла сохраняем последний подшаблон (если он не пустой)
    if current:
        print(f"\nЗавершение обработки - сохраняем последний подшаблон '{current}' (позиция {startPosition})")
        subPatterns.append(current)
        positions.append(startPosition)
    else:
        print("\nЗавершение обработки - последний подшаблон пуст")

    # выводим итоговый результат разбиения
    print("\nИтоговые подшаблоны и их позиции:")
    for sub, pos in zip(subPatterns, positions):
        print(f"    Подшаблон '{sub}' начинается на позиции {pos}")
        ...
        for j in range(i + 1, len(results)):
            ...
            if startPosition1 <= endPosition2 and startPosition2 <= endPosition1:  # концы должны быть не раньше начал сравниваемых вхождений
                ...
                intersectionPairs.append((patterns[patternIndex1 - 1], patterns[patternIndex2 - 1],
                                          intersectionPoint))  # добавляем в массив пересечений
...
def main():
    ...
    jokerSymbol = input("Введите джокер-символ: ")

    # делим шаблон с символами-джокерами на подшаблоны
    subPatterns, positions = findSubPatterns(pattern, jokerSymbol)

    ahoCorasicAlgorithm = AhoCorasicAlgorithm(subPatterns)  # инициализация алгоритма (создание автомата)
    ahoCorasicresults = ahoCorasicAlgorithm.search(text)  # поиск вхождений

    C = [0] * (
            len(text) + 1)  # массив, где C[i] -  количество встретившихся в тексте безмасочных подстрок шаблона, который начинается в тексте на позиции i

    # заполняем массив C
    print("Перебираем найденные вхождения")
    for startPosition, patternIndex in ahoCorasicresults:
        startPositionInText = startPosition - positions[patternIndex - 1]  # определяем позицию подшаблона в тексте
        if startPositionInText >= 0:
            print(
                f"Найдено вхождение ({startPosition}, {patternIndex}) на позиции {startPosition}. Увеличиваем C[{startPositionInText}]")
            C[startPositionInText] += 1
    print(f"\nИтоговый массив С: {C}")
    print(f"Число подшаблонов: {len(subPatterns)}")

    # вывод результатов
    print("\nНайденные вхождения:")
    results = []  # итоговые результаты поиска вхождений шаблона

    for i in range(len(text)):
        if C[i] == len(subPatterns) and i + len(pattern) - 1 <= len(
                text):  # если число подшаблонов на позиции i равно общему количеству подшаблонов и шаблон полностью в тексте
            print(f"Позиция {i} ")
            results.append((i, 1))  # добавляем позицию данного вхождения в результаты

    # вывод пересекающихся частей
    intersectionPairs = sorted(set(findIntersectingPatterns(results, [pattern])))
